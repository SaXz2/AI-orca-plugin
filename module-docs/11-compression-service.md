...
| 模块 | 文件 | 功能 |
|------|------|------|
| Tokenizer | `src/utils/tokenizer/` | 多模型 Token 估算、运行时校准 |
| Compression Config | `src/services/compression-config.ts` | 配置 Schema 校验、预设管理 |
| Semantic Breakpoint | `src/services/semantic-breakpoint.ts` | 语义断点检测、人工标记支持 |

### 主要改进

1. **真实 Tokenizer 支持**
   - 模型特定的估算策略（GPT/Claude/Gemini/DeepSeek）
   - 运行时偏差校准（根据 API 返回调整）
   - 5% 安全余量避免截断

2. **改进的填充策略**
   - 使用短随机 ID 避免重复字符 `<!-- pXXXX -->`
   - 支持多种填充策略（comment/whitespace/marker/none）
   - 模型特定配置

3. **配置 Schema 校验**
   - 类型安全的配置定义
   - 预设配置（deepseek/gemini/claude/gpt/general）
   - 启动时打印配置便于排查

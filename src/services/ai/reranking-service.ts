 ...
 */
export function explainScore(result: RerankResult): string {
  const breakdown = result.scoreBreakdown;
  if (!breakdown) {
    return `Score: ${(result.rerankScore * 100).toFixed(1)}%`;
  }
  
  const parts: string[] = [];
  
  if (breakdown.bm25 !== undefined) {
    parts.push(`BM25: ${breakdown.bm25.toFixed(3)}`);
  }
  if (breakdown.semantic !== undefined) {
    parts.push(`Semantic: ${breakdown.semantic.toFixed(3)}`);
  }
  if (breakdown.titleBoost && breakdown.titleBoost > 0) {
    parts.push(`Title: +${(breakdown.titleBoost * 100).toFixed(0)}%`);
  }
  if (breakdown.topicBoost && breakdown.topicBoost > 0) {
    parts.push(`Topic: +${(breakdown.topicBoost * 100).toFixed(0)}%`);
  }
  
  return `Score: ${(result.rerankScore * 100).toFixed(1)}% (${parts.join(", ")})`;
}

/**
 * Merge results from multiple rerank operations
 * Useful for combining results from different strategies
 */
export function mergeRerankResults(
  ...resultSets: RerankResult[][]
): RerankResult[] {
  const merged = new Map<number, RerankResult>();
  
  for (const results of resultSets) {
    for (const result of results) {
      const existing = merged.get(result.id);
      if (!existing || result.rerankScore > existing.rerankScore) {

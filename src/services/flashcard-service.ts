...
export function getFlashcardSystemPrompt(): string {
  return `【闪卡生成模式】用户要求生成闪卡。请按以下格式输出：
...
1. 每张卡片用 --- 分隔
2. Q: 题目/问题（必填）
3. A: 答案（普通卡片必填，选择题不需要）
4. T: 标签（可选，逗号分隔）
5. TYPE: choice 表示选择题，加 ordered 表示选项顺序固定
6. O: 选项（选择题用），在正确答案后加 (correct) 标记

卡片类型：
- 普通卡片：Q + A，适合概念解释、定义记忆
- 选择题：Q + 多个 O，适合知识点辨析、多选判断
...
【重要】内容来源：
- 优先从当前对话上下文中提取知识点
- 如果用户指定了主题，结合对话内容和该主题生成
- 只有当对话完全没有可提取的内容时，才询问用户想学习什么`;
}
